...
take the depth values from a raster (using a 50 m bathymetry grid for Flower Garden Banks) and appended to the attribute table as a new field in the points shapefile. Tools 3 and 4 deal with cleaning the point shapefile to 
only keep values within the mesophotic depth range and remove points that were not assigned values from the raster (-9999) and then delete temporary files created. 
...
suitability models for mesophotic coralline algae and/or reefs.