...
input_shp = r'C:\Users\Philip Yang\OneDrive - University of Rhode Island\NRS_528\ArcGIS_Python_Class\Class_9_Cursors\Step_3_Data\URI_Campus_Roads_OSM.shp'
...
# expression = arcpy.AddFieldDelimiters(input_shp, "fclass") + " = 'residential'"
# expression_paths = arcpy.AddFieldDelimiters(input_shp, "fclass") + " = 'path'"
#
# with arcpy.da.SearchCursor(input_shp, fields, expression_paths) as cursor:
#     for row in cursor:
#         print(u'\n {0}, {1}, {2}'.format(row[0], row[1], row[2]))
#
#
# with arcpy.da.UpdateCursor(input_shp, fields, expression_paths) as cursor:
#     for row in cursor:
#         row[2] = 10
#         cursor.updateRow(row)
#         print("\n Updated..." + str(row))
...
expression_Moore = arcpy.AddFieldDelimiters(input_shp, "name") + " = 'Mooresfield Road'"

with arcpy.da.SearchCursor(input_shp, fields, expression_Moore) as cursor:
    for row in cursor:
        print(u'\n {0}, {1}, {2}'.format(row[0], row[1], row[2]))

with arcpy.da.UpdateCursor(input_shp, fields, expression_Moore) as cursor:
        for row in cursor:
                row[2] = 40
                cursor.updateRow(row)
                print("\n Updated..." + str(row))