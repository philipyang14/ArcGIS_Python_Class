...
arcpy.env.workspace = r"C:\Users\Philip Yang\OneDrive - University of Rhode Island\NRS_528\ArcGIS_Python_Class\Class_9_Cursors"
...
#Extend code to take multiple origins - use a for loop
out_name = "radiating_line2.shp"
input_locations = [(-71.42,  41.47), (-72.42,  42.47), (-73.42,  43.47)]
def radiating_lines(out_name, input_locations):
    # Set local variables
    out_path = arcpy.env.workspace
    out_name = out_name
    geometry_type = "POLYLINE"
    ...
    # From a point (origin), let's create some radiating lines and add them to the new shapefile
    for location in input_locations:
        origin_x, origin_y = (location[0], location[1])
        distance = 1
        angle = 10  # in degrees
...
radiating_lines(out_name, input_locations)