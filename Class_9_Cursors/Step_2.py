...
input_shp = r'C:\Users\Philip Yang\OneDrive - University of Rhode Island\NRS_528\ArcGIS_Python_Class\Class_9_Cursors\Step_2_Data\Places.shp'
...
with arcpy.da.SearchCursor(input_shp, fields, expression) as cursor:
    for row in cursor:
        print(u'\n Code = {0}, Population = {1}, Town_name = {2}, x_coor = {3}, y_coor = {4}'.format(row[0], row[1], row[2], row[3][0], row[3][1]))
...
expression_S = arcpy.AddFieldDelimiters(input_shp, "name") + " LIKE 'S%' " # Clean
count = 0
with arcpy.da.SearchCursor(input_shp, fields, expression_S) as cursor:
    for row in cursor:
        print(u'\n Town_name = {0}, Population = {1}'.format(row[3], row[2]))
        count = count + 1
